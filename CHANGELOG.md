...
- Milestone 1 native client: core question and answer types, dynamic builders,
  ...
  target-selected `DefaultTransport`, recorded live API fixtures, and the
  `native-dynamic` example.
...
- Milestone 3 WASI HTTP 0.2 transport selected on `wasm32-wasip2`,
  monotonic-clock retry sleep, runnable component example, and fixture-backed
  Wasmtime smoke test.
- Milestone 4 WASI HTTP 0.3 async transport selected on `wasm32-wasip3`,
  monotonic-clock retry sleep, runnable component example, and fixture-backed
  Wasmtime smoke test on the pinned nightly toolchain.
